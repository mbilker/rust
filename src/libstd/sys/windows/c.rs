...
compat_group! {
    ...
    pub fn SleepConditionVariableSRW(ConditionVariable: PCONDITION_VARIABLE,
                                     ...
                                     Flags: ULONG) -> BOOL {
        {
            ...
            let mut timeout = dwMilliseconds as LARGE_INTEGER;
            let timeout_ptr = if dwMilliseconds == INFINITE {
                ptr::null_mut()
            } else {
            ...
            if status == STATUS_SUCCESS { TRUE } else { FALSE }
        }
    }
    ...
    pub fn TryAcquireSRWLockExclusive(SRWLock: PSRWLOCK) -> BOOLEAN {
        update_srwlock(&*(SRWLock as *mut AtomicUsize), |f, ex, sh| {
            if (f, ex, sh) == (false, 0, 0) {
                (true, 1, 0, TRUE as BOOLEAN)
            } else {
                (f, ex, sh, FALSE as BOOLEAN)
            }
        })
    }
...
// exclusive lock flag.
const EXCLUSIVE_FLAG: usize = 1 << (mem::size_of::<usize>() * 8 - 1);
const EXCLUSIVE_MASK: usize = !(SHARED_MASK | EXCLUSIVE_FLAG);
const EXCLUSIVE_SHIFT: usize = mem::size_of::<usize>() * 8 / 2;
...
fn keyed_event_handle() -> HANDLE {
    static KE_HANDLE: AtomicPtr<()> = AtomicPtr::new(INVALID_HANDLE_VALUE as *mut ());
...
pub type PHANDLE = LPHANDLE;
pub type ACCESS_MASK = ULONG;
pub type NTSTATUS = LONG;
pub type PVOID = LPVOID;

pub const STATUS_SUCCESS: NTSTATUS = 0x00000000;
pub const STATUS_NOT_IMPLEMENTED: NTSTATUS = 0xC0000002 as ULONG as NTSTATUS;
